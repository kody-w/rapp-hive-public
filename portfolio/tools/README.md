...
- **Version 4 onwards** are made by the package `rapp1_network` (rapp1-network 0.1.2), every source file of it in [rapp1_network/](rapp1_network/). [RELEASE.md](RELEASE.md) lists each file with its SHA-256 and size, the release and its commit, and the `generator` each of its pulses records.
